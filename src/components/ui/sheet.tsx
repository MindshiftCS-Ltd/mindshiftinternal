...
function SheetContent({
  ...
  VariantProps<typeof sheetVariants> & { closeClassName?: string }) {
  return (
    <SheetPortal>
      <SheetOverlay />
      <SheetPrimitive.Content data-slot="sheet-content" className={cn(sheetVariants({ side }), className)} {...props}>
        ...
        <SheetPrimitive.Close
          className={cn(
            'absolute top-4 right-4 rounded-xs text-foreground opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring',
            closeClassName,
          )}
        >
          <X className="size-4" />
